...
class WindowsAndroidSyncChainTest {
    ...
    @Test
    fun windowsPushLandsInRoomAndInboxAndAutoAppliesThroughTheWriter() = runBlocking {
    ...
    @Test
    fun onlyTheNewestClipOfABatchAutoAppliesButAllLandInTheInbox() = runBlocking {
        val transport = startEngine()
        transport.completeHandshake()
    ...
    @Test
    fun autoApplyOffKeepsThePushInTheInboxAndOffTheClipboard() = runBlocking {
    ...
    @Test
    fun pausedSyncStillReceivesIntoTheInboxButNeverAutoApplies() = runBlocking {
    ...
    @Test
    fun phoneCopyTravelsThroughCaptureGateRoomAndTheDrainToWindows() = runBlocking {
        val transport = startEngine()
        transport.completeHandshake()
    ...
    @Test
    fun pauseAndPrivateModeCloseTheOutboundGateAtEveryLayer() = runBlocking {
        val transport = startEngine()
        transport.completeHandshake()
    ...
    @Test
    fun autoAppliedWindowsClipIsNotEchoedBackToWindows() = runBlocking {
        ...
        val engine = SyncEngine(
            ...
            onRemoteClipsCommitted = { committed ->
                ...
                committed.forEach { applied ->
                    val delivered = InboxDelivery.deliver(
                        context,
                        applied.eventId,
                        applied.content,
    ...
    private fun change(text: String) =
        ClipboardChange(text, Sha256ContentHasher.hash(text), System.currentTimeMillis())
    ...
    private fun availableHeader(eventId: String, content: String, originSeq: Long) = ClipHeaderDto(
        eventId = eventId,
    ...
    private fun payloadItem(eventId: String, content: String, originSeq: Long) = ClipPayloadItemDto(
        eventId = eventId,
        originDeviceId = WINDOWS_ID,
        originSeq = originSeq,
        kind = "text",
    ...
    private class FakeWindowsTransport : SyncTransport {
        ...
        fun deliver(type: String, body: Any) {
        ...
        suspend fun awaitSent(type: String): SyncMessage = withTimeout(AWAIT_TIMEOUT_MS) {
