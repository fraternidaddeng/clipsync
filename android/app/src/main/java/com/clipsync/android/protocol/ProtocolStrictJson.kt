...
/**
 * Token-level JSON scan used by pairing and sync parsers. Rejects duplicate
 * object names, null, nesting above [ProtocolLimits.MAX_JSON_DEPTH], unescaped
 * controls, and lone surrogates — matching Windows [ScanStrictJson].
 */
object ProtocolStrictJson {
    fun scan(source: String, maxBytes: Int = ProtocolLimits.MAX_WEBSOCKET_TEXT_MESSAGE_BYTES) {
    ...
    private class StrictScanner(
        ...
        private fun scanObject(depth: Int) {
            require(depth <= ProtocolLimits.MAX_JSON_DEPTH) { "maximum depth exceeded" }
            expect('{')
            skipWhitespace()
            if (peek() == '}') {
                index++
