...
object PairingJson {
    ...
    private fun scanStrict(text: String) {
        ...
        StrictScanner(text).scanDocument()
    }

    private class StrictScanner(private val source: String) {
        private var index = 0
        ...
        private fun scanValue(depth: Int) {
            when (peek()) {
                ...
                't' -> literal("true")
                'f' -> literal("false")
                'n' -> require(false) { "null values are not allowed" }
                else -> scanNumber()
            }
        }

        private fun scanObject(depth: Int) {
            require(depth <= MAX_DEPTH) { "maximum depth exceeded" }
            expect('{')
            skipWhitespace()
            if (peek() == '}') {
                index++
        ...
        private fun scanString(): String {
            ...
            while (true) {
                ...
                require(index - start < MAX_DOCUMENT_BYTES) { "string too long" }
            }
        }
        ...
        private fun scanEscape(): Char {
            ...
            return when (val escape = source[index++]) {
                ...
                'u' -> {
                    ...
                    require(hex.all { it.isDigit() || it in 'a'..'f' || it in 'A'..'F' }) { "invalid unicode escape" }
                    index += 4
                    hex.toInt(16).toChar()
                }
                else -> throw SerializationException("invalid escape '\\$escape'")
            }
        }
        ...
        private fun requireNoLoneSurrogates(value: String) {
            ...
            while (position < value.length) {
                ...
                if (character.isHighSurrogate()) {
                    require(position + 1 < value.length && value[position + 1].isLowSurrogate()) { "lone surrogate" }
                    position += 2
                    continue
                }
