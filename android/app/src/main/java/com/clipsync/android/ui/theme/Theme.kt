...
data class ClipSyncColors(
    ...
    fun device(pairingOrder: Int): Color = deviceLadder[(pairingOrder - 1).mod(deviceLadder.size)]

    /** 着色底 for the low-chroma source box (tokens.md §4: 11% day / 12% night). */
    fun deviceBg(pairingOrder: Int): Color = device(pairingOrder).copy(alpha = if (isDark) 0.12f else 0.11f)

    /** 描边 for the source box (tokens.md §4: 24%). */
    fun deviceLn(pairingOrder: Int): Color = device(pairingOrder).copy(alpha = 0.24f)
}

val ClipSyncDayColors =
    ClipSyncColors(
        isDark = false,
        bg = Color(0xFFE2E9F2),
...
private fun m3Scheme(c: ClipSyncColors) =
    if (c.isDark) {
        darkColorScheme(
            primary = c.flow,
            onPrimary = c.onFlow,
...
private val ClipSyncTypography =
    Typography().let { base ->
 ...
 */
fun Modifier.charterCard(corner: Dp = 16.dp): Modifier =
    composed {
...
fun Modifier.charterSunken(corner: Dp = 12.dp): Modifier =
    composed {
        val c = LocalClipSyncColors.current
...
fun ClipSyncTheme(
    ...
    // 减弱动效跟随系统（P1#13）：no in-app switch, the system setting decides.
