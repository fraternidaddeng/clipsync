...
class ClipSyncDaoSqliteTest {
    ...
    @Test
    fun searchSoftDeleteAndCleanupRunOnFrameworkSqlite() = runBlocking {
    ...
    @Test
    fun outboxBatchLifecycleRunsOnFrameworkSqlite() = runBlocking {
        val db = inMemoryDatabase()
        try {
    ...
    @Test
    fun mediaGcQueriesRunOnFrameworkSqlite() = runBlocking {
        val db = inMemoryDatabase()
        try {
    ...
    ): ClipEventEntity = ClipEventEntity(
        eventId = eventId,
        originDeviceId = ORIGIN_ID,
        originSeq = seq,
        kind = kind,
