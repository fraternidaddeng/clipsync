...
/// the exact <c>netsh</c> line is shown as it will run, the profile scope defaults to Private
/// with Public as an explicit opt-in, and the UAC transparency gap (the prompt names
/// <c>netsh.exe</c>, not this app, and shows no arguments) is stated in words. Closing the
/// window any way but 确认 runs nothing.
/// </summary>
public partial class FirewallRuleWindow : Window
{
    ...
    private FirewallRuleCommand? BuildCommand()
    {
        ...
        return profiles == FirewallProfiles.None ? null : FirewallRuleCommand.Allow(profiles);
    }
    ...
    private void OnProfileChanged(object sender, RoutedEventArgs e)
    {
        ...
        if (CommandBox is not null && ConfirmButton is not null)
        {
            UpdateCommand();
        }
    }
