...
6. 默认只同步纯文本；不尝试猜测某段文本是否为密码，使用已知敏感应用黑名单、私密模式和暂停快捷键控制敏感内容。
...
| Android 后台被动捕获 -> Windows | 是，但须先完成能力引导 | Shizuku、ADB/`READ_LOGS` + 悬浮窗，或悬浮窗轮询 | 监听模式失效时按能力阶梯降级，不丢本地事件 |
| Android 收到后自动改写系统剪贴板 | 是；先尝试公开写入，厂商拒绝时再用特权回退 | `ClipboardManager.setPrimaryClip` 优先；必要时 Shizuku/悬浮窗 | 收到内容先进入 Android 收件箱，待写能力恢复后按策略应用 |
...
|---:|---|---|---|---|---|
| 1 | Shizuku 特权事件 | `IOnPrimaryClipChangedListener`（Shizuku UserService） | 通过 `IClipboard` Binder 读取；写回仅在公开写入探测失败时使用 | 安装 Shizuku、授权一次；无 Root 可用无线调试启动 | 事件驱动、无悬浮窗，首选；Shizuku 每次重启可能需恢复 |
...
1. Shizuku 模式优先使用原生 Binder 监听，UserService 死亡或系统 `clipboard` Binder 重启时自动重连；不可用时进入下一档，不循环报错。
...
5. `ClipboardWriteCoordinator` 先调用公开 `ClipboardManager.setPrimaryClip()` 并记录实际结果；只有厂商拒绝或锁屏策略阻断时才调用 Shizuku/悬浮窗写入回退。读取和写回状态必须分开显示。
...
- 远程桌面、键鼠共享、通知同步、短信同步。
- 文件夹同步、媒体图库、通用文件传输。文件传输继续使用 LocalSend。
- 公网 NAT 穿透、公共 Relay、自建云控制面板。
- 完整输入法、远程键鼠和无关的无障碍功能。Android 后台剪贴板所需的 Shizuku、ADB 授权和悬浮窗属于用户明确开启的本地能力，不作为隐式绕过；不要求 Root，也不降低 Android target SDK。无障碍服务不进入第一版核心链路，因为它只能间接识别“已复制”语义，受语言、应用和 ROM 差异影响大。
- 默认遥测、剪贴板内容日志、云端崩溃上报。
...
| Android 剪贴板能力层 | `ClipboardManager`、`WindowManager` Overlay、Shizuku API、`READ_LOGS`、分享 `Intent`、Quick Settings Tile | 用读取 `BackgroundClipboardBackend` + `ClipboardWriteCoordinator` 分别封装后台读取和写回；不把某一 ROM 行为硬编码进同步逻辑。 |
...
| Android 特权桥 | `dev.rikka.shizuku:api` + Shizuku UserService | 无 Root 时以 shell 身份访问系统 Clipboard Binder 并注册事件；仅在用户安装、启动和授权 Shizuku 后启用。 |
...
| 协议格式 | 版本化 JSON | 可读、跨 C# 与 Kotlin，不需要维护跨语言二进制代码生成。 |
...
不要让网络同步层直接调用 Android `ClipboardManager`。它只依赖受测试的读、写协调器，避免 Shizuku、悬浮窗和手动模式在业务代码中分叉：
...
ClipboardWriteCoordinator
  ...
  -> Shizuku/Overlay writer fallback（只在公开写入失败时）
```
...
- `ShizukuClipboardBackend`：绑定 Shizuku UserService，以 shell UID 访问 `IClipboard` Binder，注册/注销变化回调并读取文本；Clipboard Binder 死亡、Shizuku 断连或 API 形状不兼容时返回明确错误并重新探测。隐藏 Binder 反射只封装在该模块，按 Android API 版本写适配测试，不能散落到 UI 或同步层。它也可向写协调器提供回退写入，但不是 Android 后台写回的默认路径。
...
读取模式切换策略必须可见且可控：默认优先 Shizuku；用户可选择“自动降级到悬浮窗轮询”或“只提醒，不切换”。由于悬浮窗可能影响游戏、全屏视频或输入法，应用不得在用户未开启它的情况下偷偷启用该模式。写回策略单独设置，公开写入可用时不得因为 Shizuku/悬浮窗失效而错误降级为“只能手动复制”。
...
3. 类型：第一版只有 `text`；为未来图片预留 MIME、字节数、宽度和高度字段。
...
策略执行顺序固定为：暂停/私密模式 -> 来源黑名单 -> 类型 -> 字符/字节/尺寸 -> peer 方向 -> 保留期限。每条被拒绝的事件只记录原因码，不记录正文。
...
**技术栈**：Git、Markdown、JSON Schema、.NET 8、Android Studio、JDK 17、adb、Shizuku API、Android Instrumentation。
...
- [ ] 写完产品范围、威胁模型和协议 v1 文档。
- [ ] 写 `docs/android-background-clipboard.md`：记录 Android 10+ 限制、四档能力、权限、模式切换和已知 ROM 差异；附 KDE Connect、ClipShare、UniClipboard 的源码/文档依据链接。
- [ ] 创建 `BackgroundClipboardBackend`、`ClipboardAccessCoordinator`、`ClipboardWriteCoordinator`、`CapabilityReport` 的空实现和 fake backend，禁止同步层直接访问 Android 剪贴板。
- [ ] 写 `scripts/android-bootstrap.ps1`，只做显式、可回滚的 adb 检测和状态显示；可打印但不自动执行 `READ_LOGS` 授权命令，避免脚本在用户不知情时授予特权。
- [ ] 列出至少四台实体 Android 设备/系统版本和 ROM：AOSP/Pixel、OneUI、MIUI/HyperOS、ColorOS/OriginOS 中可获得的组合；为每台记录 API、锁屏策略、Shizuku、READ_LOGS 和 overlay 测试结果。
- [ ] 确定项目许可证（建议 Apache-2.0 或 MIT），列出第三方依赖许可证。
...
- [ ] 默认交付路径是“入站先落库”；`auto_apply_remote` 开启时先走公开写入，成功后再发不含正文的状态通知；公开写入失败才按用户已授权的 Shizuku/overlay 回退。所有路径都要写入明确的 apply 状态。
...
**技术栈**：Kotlin `ForegroundService`（`connectedDevice`）、AndroidX `ServiceCompat`、`NotificationCompat`、`ConnectivityManager.NetworkCallback`、Room、Kotlin Coroutines、`WindowManager.TYPE_APPLICATION_OVERLAY`、`Settings.canDrawOverlays`、Shizuku API/UserService、`ProcessBuilder("logcat")`、adb、Android Instrumentation、WorkManager。

**原因**：Android 10+ 不存在一个普通权限就能让后台 App 可靠**读取并监听**剪贴板的公开 API。成熟项目的通用解法是“特权事件通道优先，透明焦点窗口/轮询兜底，手动入口永远保留”；后台写入则应先使用公开 API 实测，不先引入特权。本项目是个人侧载使用，允许用户完成一次明确的 Shizuku 或 adb/overlay 引导，因此应把后台读取层作为核心功能而不是演示功能。

#### 5.1 运行时、状态机和权限引导

- [ ] 实现读取模式 `ClipboardReadMode`：`SHIZUKU_EVENT`、`ADB_LOG_OVERLAY`、`OVERLAY_POLLING`、`FOREGROUND_ONLY`；实现独立的 `ClipboardWriteMode`：`PUBLIC_API`、`SHIZUKU_FALLBACK`、`OVERLAY_FALLBACK`、`MANUAL_ONLY`；两者共享 `BackendHealth`：`READY`、`DEGRADED`、`UNAVAILABLE`、`NEEDS_USER_ACTION`。
- [ ] 在首次配对后的向导中分别检查通知、前台服务、忽略电池优化、悬浮窗、`READ_LOGS`、Shizuku Binder 和 Shizuku 授权；每一步说明其用途、风险和可跳过后果，不能把授权埋进设置深处。
- [ ] 添加用户选择：首选读取模式、是否允许自动降级、轮询间隔、是否后台自动上行、是否后台自动应用远端内容；默认首选 `SHIZUKU_EVENT`，默认不在未经许可时启用 overlay。写回默认优先公开 API，不要求用户为写回额外开启 overlay。
- [ ] `CapabilityReport` 分开持久化读/写最后成功时间、失败次数和错误码，但不得记录文本、原始 logcat 行、目标 App 名称或 Shizuku 命令输出。
...
- [ ] 集成 Shizuku Provider/API，并实现“未安装、未启动、未授权、Binder 已死、UserService 已死、Clipboard Binder 已死、API 签名不匹配”这七类稳定错误码。
- [ ] 实现独立的 Shizuku UserService：获取系统 `clipboard` Binder，按 Android API 29 到当前版本适配 `getPrimaryClip`、`setPrimaryClip`、`addPrimaryClipChangedListener` 和 `removePrimaryClipChangedListener` 的参数签名。
- [ ] UserService 以 Shizuku 提供的 shell 身份运行，严格限制 Binder API 面，只传输文本和事件 ID；不要把应用私钥、pair secret 或网络连接放进 UserService。
- [ ] 注册 `IOnPrimaryClipChangedListener` 后只把“可能变化”通知协调器，再由 backend 读取当前文本、算哈希、应用策略并落库；回调本身不直接发网络包。
- [ ] Shizuku 写入只注册为 `ClipboardWriteCoordinator` 的回退 backend；公开写入失败后才调用。任何写入前都设置短生命周期 `originEventId`/哈希抑制标记，回调到达时确认来源，防止 Android -> Windows -> Android 回环。
- [ ] 对 Shizuku/clipboard Binder 加 `linkToDeath`、指数退避重绑和健康探针；重新绑定后先刷新当前哈希，不把重绑误认为用户复制。
- [ ] 提供一键“测试后台读取”和“测试后台写入”，只使用应用生成的随机测试文本并立即清除，不读取或上传用户现有剪贴板。
...
- [ ] 不实现 Root、降低 `targetSdk`、静默 adb、隐藏悬浮窗权限、完整输入法或无障碍监听；这些不是达到本项目目标所需的默认依赖。以后若增加输入法模式，必须作为独立权限模型和安全评审项。

#### 5.7 自动化和实体机验收

- [ ] 为每个 backend 写 fake backend 单元测试：模式选择、模式切换、失败回滚、hash 去重、写回环抑制、outbox 原子落库、权限丢失。
...
- [ ] 为 Shizuku 写可跳过的设备测试：授权后读取、监听事件、远端写入、Binder 重启后恢复；测试缺少 Shizuku 时必须标为 skipped 而非假通过。
...
- Shizuku 模式在至少两种 API/ROM 组合上，Android 复制文本到 Windows 的 Wi-Fi P95 延迟不超过 1.5 秒；Windows 远端文本在 `auto_apply_remote` 开启时优先由公开 writer 写入 Android，公开 writer 失败时才验证 Shizuku 回退，且 100 次循环不产生回传。
...
- 应用被系统杀死、Shizuku 重启、网络切换和设备解锁后，不丢已落库事件、不重复上传；恢复失败时用户能从通知或设置一眼看到下一步。
...
- [ ] 对 Shizuku UserService 做最小权限审计：只暴露剪贴板读、写、listener 注册/注销和 health；它不接收网络包、不保存身份密钥、不执行任意 shell 命令。
...
- 撤销 Shizuku、overlay 或 `READ_LOGS` 授权后，应用不再尝试对应调用，状态和降级路径在一个健康检查周期内更新。
...
- [ ] 提供安装、配对、电池优化、Shizuku、adb/`READ_LOGS`、overlay 和故障排查文档；每个模式说明开机后是否需要恢复、如何停用和可观察的状态。
- [ ] `android-bootstrap.ps1` 支持列出设备、检查开发者调试/授权状态、打印可复制命令和撤销命令；默认不执行任何特权授权，也不下载或捆绑 Shizuku。
...
- 用户能在 3 分钟内从“能力不可用”页面定位到具体缺失项（Shizuku 未启动、`READ_LOGS` 未授予、悬浮窗未开启、被电池优化限制或网络不可达），而不需要阅读日志。
- 升级和回滚不会破坏配对密钥或历史数据库。
- 卸载行为清晰，不悄悄删除用户数据。
...
- 干净安装、首次配对、Shizuku 启动/授权/断连、adb `READ_LOGS` 授予/撤销、悬浮窗授予/撤销、开机恢复、屏幕关闭、锁屏/解锁、网络切换、进程被杀、通知权限拒绝、电池优化开启/关闭。
...
- [ ] Android 在 `SHIZUKU_EVENT`、`ADB_LOG_OVERLAY` 或 `OVERLAY_POLLING` 的健康状态为 `READY` 时，复制后无需打开 App 即可自动落库并同步到 Windows。
...
2. 开始阶段 5 前重新核对上述链接和当前 `targetSdk` 的行为变化，并把核对日期写入 `docs/android-background-clipboard.md`。
...
6. 不为了赶进度静默降低安全校验、降低 Android `targetSdk`、静默调用 adb/特权服务或把剪贴板正文写入日志；任何 Shizuku、`READ_LOGS` 或悬浮窗授权都必须有用户可见的前提、状态和撤销路径。
