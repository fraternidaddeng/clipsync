...
- 日期：2026-08-24（2026-08-25 状态核对至 main `d573080`）
- 分支：`cursor/implement-charter-ui-1991`（PR #5，已于 2026-08-25 经 `9129ce5` 全部并入 main）
...
> - **阶段 6 硬化**：Windows 睡眠/唤醒快速恢复已解决（`09a2d6b` + Modern Standby `25d2788`）；限流部分解决（预认证 per-IP 滑窗限流 `25d2788`，**会话内帧级速率限流仍未做**）。
> - **实体机验证不再是零，但远未达标**：2026-08-25 首轮人工 QA（真 Windows × 红米 Note 11T Pro，`docs/manual-qa-results.md`）证明了现行配对上的前台双向文本同步、回环抑制（单样本）、同文二次复制、暂停捕获，**判定「本轮不能出 RC」**；三档后台读取仍 0 台实体机验证，矩阵 S0–S4 全 `NOT_TESTED`。蓝牙备援阶段 0 spike 另有一对真机 GO 证据（工具路径，非产品路径）。
> - **本文范围外的后续新增**（写作时不存在，对比表未计）：蓝牙备援传输（ADR 0005，双端，默认关）、19 语多语言（P1#16）、基础设置两批与外观手动覆盖——见 `CHANGELOG.md`。
> - 自动化测试规模由 668 增至 **1322**（652 Android JVM + 484 跨平台核心/对端 + 186 Windows 应用层，`d573080` 时点全绿）。
...
- 测试：484 个核心/对端用例（跨平台跑）+ 186 个 Windows 专属应用层用例（WPF/DPAPI/Win32），2026-08-25 起在 windows-latest CI 实跑全绿（`d573080` 时点）。
...
| Windows 捕获/历史/策略/写回 | 实现 + 测试；2026-08-25 首轮人工 QA 在真 Windows 上触及（真实局域网 + Clash TUN 共存下同步接通），密码管理器/RDP/Office 共存与防火墙/多网卡场景未系统实测 |
| Android 手动/前台链路 | 实现 + JVM 测试；2026-08-25 首轮人工 QA 在一台真机（MIUI，D3 系统族）上验证了前台双向文本同步与分享面板上行（单一设备对，非矩阵执行） |
| Android 三档后台读取 | **代码实现 + 纯逻辑测试；0 台实体机验证（矩阵全 `NOT_TESTED`）** |
| 图片/文件同步 | **图片同步双端已落地**（protocol v2：Windows CF_DIB 捕获 → PNG 编码 → 分块传输 → 缩略图历史/详情；Android 媒体层 + Room v2 + 引擎分块 + UI，`8275ffa`；默认双端关闭）；**真机图片互拷未验证**；文件同步未做（边界） |
| 打包分发（阶段 7） | **最小分发链已落地（commit `eedf009`）**：便携 ZIP + 签名 APK 打包脚本 + SHA-256 + 一页安装文档；Releases 上传/商店上架未做 |
| 睡眠/唤醒快速恢复、帧级限流（阶段 6 硬化） | 睡眠/唤醒恢复已做（`09a2d6b` + Modern Standby `25d2788`，真机时延未实测）；限流部分做（预认证 per-IP 滑窗限流），**会话内帧级速率限流未做** |
| 蓝牙备援（IP 全不可达时 RFCOMM 续传文本，本文写作后新增） | 双端实现（ADR 0005，默认关，图片不过蓝牙）+ 阶段 0 真机 spike GO（工具路径）；产品路径整机验证未做 |
...
6. **双端原生（WPF + Compose）+ 完整设计语言。** ClipShare 是 Flutter、ClipCascade 桌面是 Python 托盘、SyncClipboard 移动端是配套壳。我们对托盘、FGS、生命周期、通知的控制是直接的系统 API；且有成文设计宪章、令牌系统、日/夜主题（含手动覆盖）、随包字体和以中文为源语言的 19 语界面（2026-08-25 起，未经真机 QA）——开源工具里 UI 品质属于罕见档。

## 五、短板 / 缺失（诚实分类）

### A. 做了但未验证（当前最大风险）

1. **实体机验证不再是零，但核心卖点仍未被真机证明。** 2026-08-25 首轮人工 QA（`docs/manual-qa-results.md`，真 Windows × 红米 Note 11T Pro）证明了现行配对上的前台双向文本同步、回环抑制（单样本）、同文二次复制、暂停捕获——并如实判定「本轮不能出 RC」（配对仪式、撤销、断线补齐、延迟样本、后台读取全部未测）。`docs/device-validation-matrix.md` 的四个 ROM 槽位仍全部 `NOT_TESTED`：三档后台读取在真机上能否读到内容、FGS 在各 OEM 电池策略下的存活、开机恢复、锁屏写入、扫码配对、真实路由器上的 UDP 发现、P95 延迟——仍只有代码和 JVM 测试。竞品最大的护城河恰恰是多年真实用户踩坑史（KDE Connect 的 ROM 兼容经验、ClipShare 的 issue 区）。**在后台读取于第一台真机跑通之前，我们的核心卖点（后台自动同步）仍只是「理论领先」。**
2. Windows 真实剪贴板生态共存（密码管理器、RDP、Office 抢占）、防火墙/多网卡场景未系统实测（首轮 QA 只覆盖了 Clash TUN 共存下的接通与 47654 监听事实）。
...
5. **阶段 6 硬化残项——多数已收口**：Windows 睡眠/唤醒会话快速恢复已解决（`09a2d6b` + Modern Standby `25d2788`）；历史导出/导入已解决（Windows `5fd7461`、Android `3c51350`、图片感知 v2 格式 `87c0016`）。**仍欠：会话内 WebSocket 帧级速率限流**（预认证 per-IP 滑窗限流已做，`25d2788`）。
6. **小项**：收件箱仍是 SharedPreferences 占位（正文在 Room 不丢）、入站通知洪泛策略仍未做；Windows 衬线字体（`2db37d2` 已随包）与空状态（`8e53c5e`）已收口，其余 UI 残项见 `ui-gap-audit.md`。
...
一句话总结对照结果：**协议正确性、能力阶梯设计、隐私边界、诚实度我们领先；真机验证、发布分发、平台覆盖我们落后（图片支持的代码差距已补上，但未经真机证明）——前者是竞品补不动的架构差异，后者全部是我们自己可以补的执行欠账。**
...
8. 会话内 WebSocket 帧级速率限流（预认证 per-IP 滑窗限流已做，`25d2788`）、收件箱 Room 化、入站通知洪泛策略。
...
这个项目的差异化不是「功能更多」——按功能清单我们永远赢不了 ClipShare。差异化是三件事：**①「手机后台复制自动到电脑」这件竞品都做得七零八落的事，我们用四档阶梯 + 诚实状态把它做成工程品；②断线补齐有协议保证而不是碰运气；③零服务器零账号的信任模型让隐私论证只有一句话：内容除了你的两台设备谁也碰不到。** 而当前最诚实的自我评价是（2026-08-25 更新）：这三件事在代码和测试层面都已成立；首轮真机人工 QA 证明了前台双向同步这一段，但**后台自动读取——核心卖点本身——在任何一台真实手机上仍未被证明**。下一步的全部价值仍集中在「验证」和「分发」，不在新功能。
