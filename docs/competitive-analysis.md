...
| 打包分发（阶段 7） | 未做（无发布产物、无签名 APK 流程、无安装文档）——**并行任务推进中（2026-08-24），未合入** |
...
3. **打包分发（阶段 7）完全未做。** 没有便携 ZIP/安装器、没有签名 APK 发布流程、没有面向用户的安装/配对/授权文档。所有竞品都有 Releases 可下载——没有分发，产品对用户来说等于不存在。*（进行中：并行任务正在做便携 ZIP/签名 APK/发布 CI，截至 2026-08-24 未合入。）*
...
2. **最小分发链**（阶段 7 裁剪版）：Windows 便携 ZIP + Android 签名 APK + 一页安装/配对/授权文档 + SHA-256 校验。不求商店上架，求「换台设备 10 分钟能装起来」。*（进行中：并行任务负责，含发布 CI；截至 2026-08-24 未合入。）*
