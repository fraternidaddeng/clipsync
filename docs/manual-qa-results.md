...
- **缺陷 3 已修复**：Windows `/v1/peer/health` 现自报 `clipboard_apply_text`（off/paused/unverified/applied/failed，姿态 + 本会话真实写入证据）；Android 通路「对端写入」据此显示已验证/已开启（就绪）或对端关闭/已暂停/写入失败（降级，注明依据）。对端未上报时仍如实显示「未探测 · 对端未上报」，不做负面猜测。
- **缺陷 4 已修复**：超限拒绝现有用户可见提示——Windows 历史页事实条（「知道了」可关，下一次成功捕获自动退场）+ 主窗隐藏时的托盘气泡；Android 自动捕获路径 Toast。均只说尺寸事实（本机保留、未截断、不同步），不含剪贴内容。
- **缺陷 5 经核对为环境差异，非默认值分歧**：两端代码默认「图片同步」均为**关**（Windows `image_sync` 缺省 False；Android `sync.image_sync` 缺省 false，见 ADR 0004）。QA 的 Windows 机器此前手动开启并持久化在设置库里，所以显示「开」。默认值本就对齐，无代码改动；下轮 QA 若需双向图片验收，请在两端显式同开。
- 缺陷 1、2、6 不在本次修复范围（缩略图单测、幽灵设备撤销、CI 核实）。
