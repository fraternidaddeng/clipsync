...
- 首轮「本轮已知限制」1–6 此前已全部在 main 修复并闭环（见上方「修复附注」与三则更新）。发布收尾（把 CHANGELOG Unreleased 归入 v0.1.0、打 tag 触发 `release.yml`、定稿发布说明）为剩余步骤。**2026-08-26 同日更新**：CHANGELOG 已归档为 `[0.1.0] - 2026-08-26`、发布说明已定稿为 `docs/releases/v0.1.0.md`（原 `v0.1.0-draft.md`）；剩余仅打 tag——先 `v0.1.0-rc.1` 验证 `release.yml` 首次实跑，再 `v0.1.0` 正式发布。
