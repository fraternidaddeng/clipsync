...
| 「测试后台读取」报「特权读取服务已断开」（PRIV_HOST_USERSERVICE_DEAD） | 读取测试会等新拉起的服务连上（最多约 12 秒），所以正确顺序是：① 电脑端点「启动特权直读」（或重跑启动命令）② 等它报成功 ③ 回手机**直接再点一次「测试后台读取」**——测试自己会等服务上线，通过即转就绪。仍失败才说明宿主没起来：回查电脑端启动结果与无线连接状态 |
