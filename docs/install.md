...
- **端口会漂**：手机息屏、切网或重启后「IP 地址和端口」常会变化，连接失败时回无线调试页核对当前值。
...
| 无线配对：已配对但连接失败 | ① 「连接 IP:端口」要填「无线调试」页**顶部**的「IP 地址和端口」，不是配对弹窗里的端口 ② 手机息屏/切网后端口会变，回无线调试页核对当前值 ③ 手机或电脑重启后重新「连接」即可，配对不用重来 |
