...
| 电脑说宿主已启动，手机通路页还是不就绪 | ① 回到剪剪相传（页面恢复会自动重新探测）或手动点「重新探测」 ② 若提示「特权读取服务已断开」，先「重新探测」等它自动重连；仍无效说明宿主真死了（手机重启/系统回收/无线会话失效），在电脑端重跑一次启动命令（Windows 一键「启动特权直读」）③ 无线场景先确认电脑端无线连接还活着（见上一行） |
