...
5. §4 Modern Standby power source (drop-in alongside existing resilience controller) — **open**.
6. §4 rate limiter, ~~v1 strict-parse fixtures~~ (done `fed1b6f`+`bd0d780`), E2E harness, ~~static analysis~~ (done `f21c6cb`…`c6adf20`), docs — **rate limiter, E2E harness, audit docs still open**.
