...
| A10 | 入站通知策略（`InboundNotifyPolicy` 去重/静音） | ~~Missing~~ → **已收口（通知洪泛上限 2026-08-25）** | Low | 审计时点：按事件 ID 更新式通知（不会无限堆叠），收件箱上限 50 条。**2026-08-25 注：`InboxNotificationGate` 固定窗洪泛上限落地——每 30 秒窗口前 5 条逐条通知，其余合并为一张就地更新的计数卡（不再刷满通知栏、也不再任由系统限流静默丢弃）；录入与自动写入永不受限，单测 + 投递级合并测试覆盖。** | 已完成（JVM 测试级）。 |
| A11 | 收件箱 Room 化 | ~~Partial~~ → **已收口（`f928537`）** | Low | 审计时点：`KeyValueClipInbox` 是 SharedPreferences JSON 占位（代码注释已声明），事件正文本身在 Room 里不丢。**2026-08-25 注：`RoomClipInbox` 落地——通知复制动作按事件 ID 直查 Room 历史行，SharedPreferences 不再留第二份明文（旧 50 条残留一次性清除），删除历史即令复制动作失效。** | 已完成。 |
...
| W5 | WebSocket 帧速率限制（`SlidingWindowRateLimiter`） | ~~Missing（对比 stage-4）~~ → **已收口（`25d2788` + 会话内帧级限流 2026-08-25）** | Low-Medium | 审计时点：认证节流 `AuthThrottle` 在，帧级限流缺失。**2026-08-25 注：`SlidingWindowRateLimiter` 已移植，用于预认证的连接接受与配对确认 per-IP 滑窗限流；会话内（认证前后一体）帧级限流随后落地——`FrameRateBudget` 固定窗（默认 10 000 帧/分钟，容 16 MiB 图片分块流仍有宽裕）在解析/重放哈希之前计数，超限发可重试 RATE_LIMITED 后关会话，单元 + 洪泛集成测试覆盖；配套护栏 want_ranges 受限拉取、超大帧 PAYLOAD_TOO_LARGE 与 Android 入站帧大小上限（`479eba0`）。** | 已完成（JVM/回环测试级）。 |
