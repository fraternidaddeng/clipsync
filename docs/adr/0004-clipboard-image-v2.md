...
- 状态：接受（2026-08-28 两次修订：图片同步与 `auto_apply_images` 的产品默认值先后由关改开，见「修订记录」；其余决策不变）
- 日期：2026-08-20
...
- 图片同步默认开启（2026-08-28 修订；原为默认关闭）；`auto_apply_images` 与文本自动应用独立，且同日修订为默认开启（原为默认关闭）——收到的图片一律入历史，自动写入本机剪贴板默认发生，用户显式关闭（持久化 false）仍被尊重，暂停/私密仍照常拦下自动写入。图片读/写能力不得从文本 `READY` 推断。
...
- 2026-08-28：图片同步的产品默认值由「默认关闭」改为「默认开启」。产品裁决：「图片同步这种功能应该默认打开，这是产品的完整体验，而不是蓝牙那种备选方案」——图片同步属于产品的完整体验，与 ADR 0005 蓝牙备援（备选方案，维持默认关）性质不同。变更范围仅限产品默认的选择倾向，安全语义一律不动：
  ...
  - `auto_apply_images`（远端图片自动写入本机剪贴板）维持默认关闭：同步与入历史默认发生，但自动改写本机剪贴板仍是隐私敏感的显式选择，与文本自动应用继续独立。（同日稍后经产品裁决再次修订为默认开启，见下一条。）
- 2026-08-28（同日随后）：`auto_apply_images`（远端图片自动写入本机剪贴板）的产品默认值也由「默认关闭」改为「默认开启」。产品裁决：「无所谓啊，本来截图就是我自己截的，默认开开」——与图片同步同理，属于产品完整体验的一部分。变更范围同样仅限产品默认的选择倾向：
  - Android `sync.auto_apply_images` 与 Windows `auto_apply_images` 设置缺省或不可解析时按「开」解析（与 `image_sync` / `auto_apply_remote` 同规则）；用户显式关闭的持久化值仍被尊重。
  - 拦截语义不动：暂停同步仍双杀文本与图片的自动写入（Android `InboxDelivery.autoApplyImagesAllowed`、Windows `RemoteApplyDecision`），Windows 私密模式照旧一并停下自动写入；图片写入门与文本自动应用继续互相独立；回声抑制、MIME 魔数/尺寸/哈希校验等安全检查原样保留。
