...
- 状态：已接受（阶段 0 实体机 spike 于 2026-08-25 判定 **GO**，全部门槛通过，限制表数值已用实测校准——见 `docs/bluetooth-phase0-report.md`；双端软件与单测已落地、默认关闭；产品路径的实体机验证待做——见 `docs/bluetooth-fallback-plan.md` 阶段 5）
- 日期：2026-08-25
...
| 项 | 值/说明 |
|---|---|
| 实际吞吐 | BR/EDR RFCOMM 约 50–200 KB/s（视适配器/干扰）。阶段 0 实测（Realtek 适配器 × Redmi Note 11T Pro，bt1 模式 256 KiB 档，三轮）：上/下行 150–180 KiB/s，RTT 中位约 31 ms——好于本表保守估值；典型剪贴板文本（<4 KiB）应在 1–2 s 内送达；1 MiB 上限文本按实测吞吐约 6–7 s，最坏数十秒（1 MiB 档未实测，阶段 5 补） |
| 建连延迟 | 阶段 0 实测 `connect_ms` 0.7–2.2 s（三轮递增波动，均远低于 5 s 门槛）；不得假设亚秒建连，重连与拨号超时（30 s 看门狗）必须保留 |
| 链路 MTU | RFCOMM 帧由 L2CAP MTU 决定（常见 ~1000 字节，栈自动分段），应用层不感知；应用帧上限 7 MiB 明文 |
| 有效距离 | 约 10 m 级，隔墙衰减明显（阶段 0 在 1–2 m 基准距离测得，距离/隔墙属阶段 5） |
| 并发 | 单蓝牙会话；服务端拒绝第二连接 |
...
代价：Windows 需引入 WinRT 投影目标框架并新增程序集；需要维护 bt1 握手/帧层的跨语言测试向量；蓝牙窗口内的图片是明确的功能空洞；实体机验证矩阵扩大（适配器差异、OEM 蓝牙栈差异——阶段 0 只覆盖 Realtek × Xiaomi 一对）。按仓库规则，没有实体机证据不得标 `READY`：阶段 0 spike 证据（`docs/bluetooth-phase0-report.md`）解决的是「值不值得做」，READY 还需阶段 5 的产品路径矩阵。
